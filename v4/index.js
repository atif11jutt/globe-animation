...
const initThreeJS = () => {
  ...
  const arcData = [
    { start: { lat: 40, lon: -74 }, end: { lat: 51, lon: 0 }, color: 0xff0000 },
    { start: { lat: 30, lon: -95 }, end: { lat: 20, lon: 0 }, color: 0x00ff00 },
    {
      start: { lat: 20, lon: 10 },
      end: { lat: 40, lon: -120 },
      color: 0x0000ff,
    ...
    {
      start: { lat: -20, lon: 120 },
      end: { lat: -30, lon: 150 },
      color: 0xff00ff,
    },
    // Add more arcs connecting different regions
    {
      start: { lat: 40, lon: -74 },
      end: { lat: -33, lon: 151 },
      color: 0xff6600,
    ...
    { start: { lat: 20, lon: 10 }, end: { lat: 10, lon: 77 }, color: 0xff0066 },
    {
      start: { lat: 60, lon: -50 },
      end: { lat: 55, lon: -105 },
      color: 0x6600ff,
  ...
  const drawArcs = () => {
    arcData.forEach((arc) => {
      const start = calcPosFromLatLonRad(arc.start.lon, arc.start.lat);
      const end = calcPosFromLatLonRad(arc.end.lon, arc.end.lat);
      const curve = new THREE.QuadraticBezierCurve3(
        start,
        new THREE.Vector3(
          start.x + (end.x - start.x) / 2,
          start.y + (end.y - start.y) / 2 + 10,
  ...
  const render = () => {
    ...
    arcData.forEach((arc, index) => {
      ...
      const lineObject = scene.children.find(
      ...
      if (lineObject) {
        ...
        const endSphere = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: arc.color })
        );
        endSphere.position.copy(end);
        scene.add(endSphere);

        const curveObject = new THREE.Line(geometry, material);
        curveObject.name = `arc_${index}`;
        scene.add(curveObject);
      }
    });
